  ...
  try {
    ...
    if (userloggId._id != userLoggin)
      return res
        ...
        .send({
          status: false,
          message: "logedin user is not authorized To create book",
        });
...
let AuthByQuery = async function (req, res, next) {
  try {
    ...
    if (!/^[0-9a-fA-F]{24}$/.test(requestBookId)) {
      return res
        .status(400)
    ...
    if (findBookID.userId != userLogin)
      return res
        .status(403)
        .send({ status: false, message: "logedin user is not authorized " });
