...
const createReview = async function (req, res) {
  try {
    ...
    if (!req.body.reviewedBy) req.body.reviewedBy = "Guest";
      ...
      return res
        .status(400)
        .send({ status: false, message: "Please provide reviewedBy name" });
    } else if (/\d/.test(req.body.reviewedBy)) {
      return res
        .status(400)
        .send({ status: false, message: "reviewedBy cannot have numbers" });
    } else {
      req.body.reviewedBy;
    }
    ...
    if (!data.rating) {
      return res
        .status(400)
        .send({ status: false, message: "Rating field is required" });
    }
    ...
    if (data.isDeleted === true)
      return res
        ...
        .send({
          status: false,
          message: "This isDeleted field value sholud be in false",
        });
    ...
    const countData = await reviewModel.countDocuments({
      bookId: BookId,
      isDeleted: false,
    });
...
const updateReview = async function (req, res) {
  try {
    ...
    if (!/^[0-9a-fA-F]{24}$/.test(reviewId)) {
      return res
        .status(400)
        .send({ status: false, message: "ReviewId format isn't correct" });
    }
    ...
    if (!review || review.isDeleted == true) {
      return res
        .status(404)
        .send({ status: false, message: "No Review Found by this reviewId" });
    }
    ...
    if (Object.keys(data).length == 0) {
      return res
        ...
        .send({
          status: false,
          message: "Please provide your Review details in body",
        });
    }
    ...
    if (data.reviewedBy) {
      if (!data.reviewedBy.match(/^[a-zA-Z. ]+$/)) {
        return res
          .status(400)
          .send({ status: false, message: "Reviewer can't be a number" });
      }
    }
    ...
    if (data.review) {
      if (data.review.trim().length == 0) {
        return res
          .status(400)
          .send({ status: false, message: "Please provide review" });
      }
    }
...

    {
      return res
        .status(200)
        .send({ status: true, message: "Books list", data: data2 });
    }
  } catch (err) {
    {
      return res
...
const deleteReview = async function (req, res) {
  try {
    ...
    let countData1 = await reviewModel.countDocuments({
      bookId: book_id,
      isDeleted: false,
    });
