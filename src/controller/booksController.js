...
const createBook = async function (req, res) {
    try {
        ...
        if (!data.excerpt || data.excerpt.trim().length == 0) {
            return res
                .status(400)
                .send({ status: false, message: "Excerpt field is required" });
        }

        if (!data.userId || data.userId.trim().length == 0) {
            return res
                .status(400)
                .send({ status: false, message: "UserId field is required" });
        }

        if (!data.ISBN || data.ISBN.trim().length == 0) {
            return res
                .status(400)
                .send({ status: false, message: "ISBN field is required" });
        }

        if (!data.category || data.category.trim().length == 0) {
            return res
                .status(400)
                .send({ status: false, message: "Category field is required" });
        }

        if (!data.subcategory || data.subcategory.length == 0) {
            return res
                .status(400)
                .send({ status: false, message: "Subcategory field is required" });
        }

        if (!data.releasedAt || data.releasedAt.length == 0) {
            return res
                .status(400)
                .send({ status: false, message: "releasedAt field is required" });
        }
        if (data.reviews) {
            if (!data.reviews == 0) {
                return res.status(400).send({
                    status: false,
                    message: "Reviews field should be in default 0",
                });
            }
        }
        if (data.isDeleted) {
            if (!data.isDeleted == false) {
                return res.status(400).send({
                    status: false,
                    message: "isDeleted field should be in by default false",
                });
            }
        }
        ...
        if (titleCheck) {
            return res.status(400).send({
                status: false,
                message: "Title is already registerd, try anothor",
            });
        }
        ...
        if (isbnCheck) {
            return res.status(400).send({
                status: false,
                message: "ISBN is already registerd, try anothor",
            });
        }
        ...
        if (!/^(?=(?:\D*\d){10}(?:(?:\D*\d){3})?$)/.test(data.ISBN)) {
            return res.status(400).send({
                status: false,
                message:
                    "ISBN should be in Number type and its length should be in 13 digits",
            });
        }

        if (
            !/^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$/.test(data.releasedAt)
        ) {
            return res.status(400).send({
                status: false,
                message: "ReleasedAt format should be in YYYY-MM-DD",
            });
        }
        ...
        if (!/^[0-9a-fA-F]{24}$/.test(userdata)) {
            return res
                .status(400)
                .send({ status: false, message: "userId format isn't correct" });
        }
    ...
    try {
        if (req.query.userId) {
            ...
            if (!userIdCheck) {
                return res
                    .status(404)
                    .send({ status: false, message: "userId is not exist" });
            }

            let bookData = await booksModel
                .find({ userId: userIdCheck, isDeleted: false })
                .select({
                    _id: 1,
                    title: 1,
                    excerpt: 1,
            ...
            if (bookData.length == 0) {
                return res.status(404).send({
                    status: false,
                    message: "No such Books are found for this userId",
                });
            } else {
                return res
                    .status(200)
            ...
            let categoryCheck = await booksModel
            ...
            if (categoryCheck.length == 0) {
                return res.status(404).send({
                    status: false,
                    message: "No such similar books are found by the category",
                });
            } else {
                return res
                    .status(200)
                    .send({ status: true, message: "Books List", data: categoryCheck });
            }
        } else if (req.query.subcategory) {
            let subcategory = req.query.subcategory;

            let subcategoryCheck = await booksModel
                .find({ subcategory: subcategory, isDeleted: false })
                .select({
                    _id: 1,
                    title: 1,
                    excerpt: 1,
            ...
            if (subcategoryCheck.length == 0) {
                return res.status(404).send({
                    status: false,
                    message: "No such similar books are found by the subcategory",
                });
            } else {
                return res.status(200).send({
                    status: true,
                    message: "Books List",
                    data: subcategoryCheck,
                });
            }
        } else {
            ...
            return res
                .status(200)
                .send({ status: true, msg: "success", data: finaldata });
        }
    } catch (err) {
...
const getBookById = async function (req, res) {
    try {
        ...
        if (!/^[0-9a-fA-F]{24}$/.test(book_id)) {
            return res
                .status(400)
                .send({ status: false, message: "BookId format isn't correct" });
        }

        let checkBook = await booksModel.findOne({
            _id: book_id,
            isDeleted: false,
        });
        ...
        const getReviewsData = await reviewModel.find({
...
const updateBook = async function (req, res) {
    try {
        ...
        if (!book || book.isDeleted == true) {
            return res
                .status(404)
                .send({ status: false, message: "No Book Found by this BookId" });
        }
        ...
        if (checkBook) {
            return res
                .status(400)
                .send({ status: false, message: "Title is already used, try anothor" });
        }
        ...
        if (checkBook2) {
            return res
                .status(400)
                .send({ status: false, message: "ISBN is already used, try another" });
        }
        ...
        if (data.title) {
            if (data.title.trim().length == 0) {
                return res
                    .status(400)
                    .send({ status: false, message: "Title requried" });
            }
        }
        if (data.excerpt) {
            if (data.excerpt.trim().length == 0) {
                return res
                    .status(400)
                    .send({ status: false, message: "Excerpt requried" });
            }
        }
        if (data.ISBN) {
            if (!/^(?=(?:\D*\d){10}(?:(?:\D*\d){3})?$)/.test(data.ISBN)) {
        ...
        let updatedBook = await booksModel.findOneAndUpdate(
            { _id: bookId },
            {
                $set: {
                    title: data.title,
...
const deleteBookById = async function (req, res) {
    try {
        ...
        const deleteBookData = await booksModel.findOneAndUpdate(
            { _id: checkBook._id },
...

        {
            return res
                .status(200)
                .send({ status: true, message: "Book is deleted succesfully" });
        }
    } catch (err) {
        {
